...
local function loadVehicles(id, type)
    ...
    ESX.TriggerServerCallback("xGarage:loadVehicles", function(data)
        ...
        for _,v in pairs(data) do
            if ESX.Game.IsSpawnPointClear(vector3(xGarage.Garage[type].posCar[_].x, xGarage.Garage[type].posCar[_].y, xGarage.Garage[type].posCar[_].z), 1) then
                if not IsModelInCdimage(v.model) then return end
                RequestModel(v.model)
                while not HasModelLoaded(v.model) do
                    Citizen.Wait(10)
                end
                ESX.Game.SpawnLocalVehicle(v.model, vector3(xGarage.Garage[type].posCar[_].x, xGarage.Garage[type].posCar[_].y, xGarage.Garage[type].posCar[_].z), xGarage.Garage[type].heading, function(car) 
                                ...
                                onSelected = function()
                                    ESX.TriggerServerCallback("xGarage:deleteCar", function(can) 
                                        if can then
                                            ...
                                            local vehicle = CreateVehicle(v.name, posExit.x, posExit.y, posExit.z, GetEntityHeading(PlayerPedId()), true, false)
